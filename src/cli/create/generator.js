...
export default class extends Base {
  prompting () {
    ...
    const prompts = [
      ...
      },
      {
        ...
        name: 'folder',
        message: ({ name }) =>
          `In which folder do you want to put the ${chalk.yellow(name || this.options.componentName)} component?`,
      ...
      },
      {
        ...
        name: 'generalParams',
        message: ({ name, folder }) => {
          const p = chalk.yellow(
            path.join(folder, name || this.options.componentName)
          )
  ...
  configuring () {
    ...
    const name = upperFirst(
      camelCase(this.answers.name || this.options.componentName)
    )
    ...
    for (var param in params) {
      context[param] = !!~finalParams.indexOf(params[param])
    }
  ...
  writing () {
    ...
    this.log(
      chalk.green(`The ${chalk.yellow(componentPath)} component created!`)
    )
  }
}
