...
export default class extends Base {
  ...
  _promptComponent (availableComponents) {
    ...
    return Promise.all([selectPort(), this.prompt(prompts)]).then(([
      port,
      answers
    ]) => {
      this.port = port
  ...
  prompting () {
    ...
    if (componentName) {
      ...
      if (!availableComponents.includes(component)) {
        this.log(
          chalk.red(`Component ${component} not found in ${packagesPath}`)
        )
    ...
    const { config, templatePath } = loadConfig({
