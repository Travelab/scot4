...
function formatSegmentPart (l, segPart) {
...
function calcPropsSegment (l, idx, segments, directionColor, warnColor, segment, arriveIata) {
	...
	const nextSegSrc = idx < segSize ? segments[idx + 1].arrive : null
	...
	return {
		...
		data: {
			depart: formatSegmentPart(l, segSrc),
...
const OrderTicketsInfo = ({ $, l, cq, state, actions, ditch, args, ...props }) => {
	...
	const flights = offer.flights.map((flight, idx) => {
		...
		const renderedGroups = ticketGroups.map((group, gid) => {
			const items = group.map((propsItem, i) => (
				propsItem.meta ? <Segment key={`s${i}`} {...propsItem} t414={t414}/> : <Transfer key={`t${i}`} {...propsItem} t414={t414}/>
			))
