...
export class Query extends MPS {
	...
	static make (filter, channel) {
		return new Query(filter, channel)
	}
	...
	_handleMessage (event) {
		...
		this._processMessage(data.data || {}, data.meta || {})
	}

	_processMessage (data, meta) {
		this._offersHandler.process(data, meta)

		this._buildResult()
	}

	_buildResult () {
		if (this._channel === 'clickavia') {
			this._result = this._offersHandler.offers
		} else {
