...
// Следит за изменением и сообщает OB о необходимости «применить» фильтр
function* watchFilterChanges () {
	while (true) {
		const { payload } = yield take(constants.applyFilter)
		// process filter
		let filter = {
			...payload,
			consist: payload.consist.split(',').map(Number)
		}
		obc.setFilter(filter)
	}
}
