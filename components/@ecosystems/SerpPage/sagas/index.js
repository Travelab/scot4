...
const { fork, take, put, call, cancel } = sagaEffects
...
function* putActionsToFillSearchBar (filters, serpPageDitch) {
	const searchBarDitch = serpPageDitch.getDitch('searchBar')
...
function* checkOBCStatus (serpPageDitch) {
...
function* watchOBChanges (serpPageDitch) {
	while (true) {
		yield take(channel)

		yield call(checkOBCStatus, serpPageDitch)
	}
}

function* watchLifecycle (serpPageDitch) {
	const { constants } = serpPageDitch

	while (true) {
		yield take(constants.componentWillMount)

		const watcher = yield fork(watchOBChanges, serpPageDitch)
		...
		yield call(putActionsToFillSearchBar, searchFilters.searchBar, serpPageDitch)
...
export default function* (ditch) {
	...
	const { header, filters, searchResults, fakeProgressBar, searchBar } = ditch.getDitches()
