...
function* putActionsToFillSearchBar (filters) {
	const searchBarDitch = caIndexPageDitch.getDitch('searchBarDitch')
...
export default function* (ditch) {
	caIndexPageDitch = ditch
	const { searchBar } = ditch.getDitches()
	yield [
		fork(watchLifecycle),
		fork(searchBarSaga, ditch.getDitches().searchBarDitch)
	]
}